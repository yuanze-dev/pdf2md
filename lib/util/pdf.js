...
exports.parse = async function parse (docOptions, callbacks) {
  ...
  for (let j = 1; j <= pdfDocument.numPages; j++) {
    ...
    if (Object.keys(hashTable).length < 10) {
      hashTable = findPageNumbers(hashTable, page.pageIndex, textContent.items)
    } else {
      pageDetails = findFirstPage(hashTable)
      break
    }
  }
  ...
  for (let j = 1; j <= pdfDocument.numPages; j++) {
    ...
    if (page.pageIndex >= pageDetails.pageIndex) {
      textContent = removePageNumber(textContent, pageDetails.pageNum)
    }
