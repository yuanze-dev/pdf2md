...
const searchArea = (start, end, pageIndexNumMap, pageIndex, items) => {
  for (let x = start; x < end; x++) {
    if (isNumber(items[x].str)) {
      if (!pageIndexNumMap[pageIndex]) {
        pageIndexNumMap[pageIndex] = []
      }
      pageIndexNumMap[pageIndex].push(Number(items[x].str))
    }
  }
...
exports.findPageNumbers = (pageIndexNumMap, pageIndex, items) => {
  ...
  const topAreaResult = searchArea(0, topArea, pageIndexNumMap, pageIndex, items)
  return searchArea(bottomArea, items.length, topAreaResult, pageIndex, items)
}
...
exports.findFirstPage = (pageIndexNumMap) => {
  ...
  for (let x = 0; x < keys.length; x++) {
    ...
    let pageDetails
    if (x > 0) {
      pageDetails = Object.entries(pageIndexNumMap)[x - 1]
    } else {
...
exports.removePageNumber = (textContent, pageNum) => {
  ...
  filteredContent.items = filteredContent.items.filter((item, index) => {
    ...
    if (isAtTop || isAtBottom) {
      return Number(item.str) !== Number(pageNum)
    }
    return item
  })
