...
const { removePageNumber, isNumber } = require('./string-functions')
...
exports.parse = async function parse (docOptions, callbacks) {
  ...
  for (let j = 1; j <= pdfDocument.numPages; j++) {
    ...
    const textItems = textContent.items.map((item, index) => {
      ...
      let text = item.str
      // Assumption: page num is the last element based on ExamplePdf
      if ((index === textContent.items.length - 1) && isNumber(item.str)) {
        text = removePageNumber(j, Number(item.str))
      }
